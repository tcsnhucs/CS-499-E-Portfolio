...
async function loadCSVData() {
  try {
    const response = await fetch('./aac_shelter_outcomes.csv');
    const csvText = await response.text();
    const results = Papa.parse(csvText, { header: true });
    allAnimals = results.data.filter(row => row.animal_id); // Filter out empty rows
    
    // Convert numeric columns
    allAnimals = allAnimals.map(animal => ({
      ...animal,
      age_upon_outcome_in_weeks: parseInt(animal.age_upon_outcome_in_weeks) || 0,
      location_lat: parseFloat(animal.location_lat) || 30.75,
      location_long: parseFloat(animal.location_long) || -97.48
    }));
    ...
    document.querySelector('.table-section').innerHTML = 
      '<h3>Animal Data Table</h3><p class="loading">Error loading data. Make sure aac_shelter_outcomes.csv is in the cs340-app folder.</p>';
  }
}
